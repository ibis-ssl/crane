...
def test_free_kick_distance(rcst_comm: Communication):
    ...
    rcst_comm.send_blue_robot(
        0, ball_x + 0.3, ball_y, math.radians(180)
    )  # 300mm - 違反
    rcst_comm.send_blue_robot(
        1, ball_x + 0.4, ball_y + 0.2, math.radians(180)
    )  # 400mm - 違反
    rcst_comm.send_blue_robot(
        2, ball_x + 0.6, ball_y, math.radians(180)
    )  # 600mm - 合法
    rcst_comm.send_blue_robot(
        3, ball_x, ball_y + 0.7, math.radians(270)
    )  # 700mm - 合法
    ...
    assert violating_count == 0, (
        f"Blue robots ({violating_count}) violated free kick distance rule"
    )

    # 違反が検出されなかったことを確認
    violation_detected = rcst_comm.observer.customized().get_result(
        "blue_robots_too_close"
    )
    assert violation_detected is False, (
        "Blue robots did not maintain proper distance during free kick"
    )
...
def test_free_kick_distance_blue(rcst_comm: Communication):
    ...
    rcst_comm.send_yellow_robot(
        0, ball_x - 0.35, ball_y, math.radians(0)
    )  # 350mm - 違反
    rcst_comm.send_yellow_robot(
        1, ball_x - 0.55, ball_y + 0.3, math.radians(0)
    )  # 550mm - 合法
    ...
    assert violating_count == 0, "Yellow robots violated free kick distance rule"
    assert (
        rcst_comm.observer.customized().get_result("yellow_robots_too_close") is False
    )
