...
def test_ball_placement_accuracy(rcst_comm: Communication):
    ...
    print(f"Ball placement success rate: {successful_placements}/{total_placements} ({success_rate:.1%})")
    
    # 最低80%の成功率を要求
    assert success_rate >= 0.8, f"Ball placement accuracy insufficient: {success_rate:.1%}"
...
def test_ball_placement_speed(rcst_comm: Communication):
    ...
    assert completion_time <= max_time, f"Ball placement took too long: {completion_time:.2f}s"
