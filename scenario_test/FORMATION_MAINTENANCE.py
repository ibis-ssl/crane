...
def test_defensive_formation(rcst_comm: Communication):
    ...
    for i in range(int(test_duration / measurement_interval)):
        ...
        print(f"Formation quality at {(i+1)*measurement_interval:.1f}s: {formation_quality:.2f}")
    ...
    # 評価基準
    assert average_formation_quality >= 0.7, f"Average formation quality insufficient: {average_formation_quality:.2f}"
    assert min_formation_quality >= 0.5, f"Formation broke down too much: {min_formation_quality:.2f}"
...
def test_offensive_formation(rcst_comm: Communication):
    ...
    for i in range(int(test_duration / measurement_interval)):
        ...
        print(f"Offensive spread at {(i+1)*measurement_interval:.1f}s: {spread_quality:.2f}")
    ...
    # 評価基準
    assert average_spread >= 0.6, f"Offensive formation spread insufficient: {average_spread:.2f}"
...
def test_formation_adaptation(rcst_comm: Communication):
    ...
    assert avg_movement >= 0.3, f"Insufficient formation adaptation: {avg_movement:.2f}m average movement"
    assert total_movement >= 1.0, f"Team did not adapt sufficiently: {total_movement:.2f}m total movement"
    
    # 過度な移動も問題（フォーメーションの崩壊）
    assert avg_movement <= 2.0, f"Excessive movement suggests formation breakdown: {avg_movement:.2f}m"
