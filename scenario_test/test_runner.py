...
class ScenarioTestRunner:
    ...
    def run_test(self, test_file: str) -> Tuple[bool, Dict]:
        ...
        pytest_args = [
            test_file,
            '--vision_port', str(self.vision_port),
            '--logging',
            '-v',
            '--tb=short'
        ]
        ...
        try:
            ...
            if success:
                ...
                print(f"❌ {test_name} failed in {duration:.2f}s (exit code: {exit_code})")
    ...
    def generate_report(self, summary: Dict, output_file: str = None):
        ...
        if output_file is None:
            output_file = f"scenario_test_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    parser.add_argument('--group', choices=['rule_compliance', 'basic_actions', 'quick_smoke', 'all'], 
                       help='テストグループの選択')
    ...
    runner = ScenarioTestRunner(
        ...
        log_recorder=args.log_recorder
    )
