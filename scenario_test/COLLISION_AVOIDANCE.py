...
def test_robot_robot_collision_avoidance(rcst_comm: Communication):
        ...
        for i, (team1, robot1) in enumerate(all_robots):
            for team2, robot2 in all_robots[i+1:]:
                ...
                if distance < 0.18:  # 180mm
                    print(f"Collision detected: {team1} robot vs {team2} robot, distance: {distance:.3f}m")
    ...
    # 結果評価
    collision_occurred = rcst_comm.observer.customized().get_result("collision_detected")
    ...
    assert overall_min_distance >= 0.18, f"Robots got too close: {overall_min_distance:.3f}m"
...
def test_dynamic_obstacle_avoidance(rcst_comm: Communication):
            ...
            for robot_id in [1, 2, 3]:
                if robot_id in yellow_robots:
                    ...
                    distance = calc.distance(
                        main_robot.x, main_robot.y, 
                        obstacle_robot.x, obstacle_robot.y
                    )
    ...
    if path_points:
        ...
        avg_collision_risk = sum(collision_risks) / len(collision_risks) if collision_risks else float('inf')
        ...
        # 評価基準
        assert distance_to_goal <= 0.5, f"Did not reach goal: {distance_to_goal:.2f}m away"
        assert min_collision_risk >= 0.20, f"Got too close to obstacles: {min_collision_risk:.3f}m"
...
def test_wall_boundary_avoidance(rcst_comm: Communication):
    ...
    for i, (start_x, start_y, target_x, target_y, description) in enumerate(boundary_tests):
        ...
        if 0 in world.get_yellow_robots():
            ...
            final_distance_to_target = calc.distance(robot.x, robot.y, target_x, target_y)
            ...
            if len(boundary_violations) == 0 and final_distance_to_target <= 0.5:
                ...
                if boundary_violations:
                    print(f"    Violations: {boundary_violations[:3]}...")  # 最初の3つのみ表示
        else:
    ...
    print(f"Boundary avoidance success rate: {successful_tests}/{total_tests} ({success_rate:.1%})")
...
if __name__ == "__main__":
    ...
    goal_distance, obstacle_distance, path_length = test_dynamic_obstacle_avoidance(rcst_comm)
    ...
    print(f"Dynamic obstacle avoidance - Min obstacle distance: {obstacle_distance:.3f}m")
