...
def test_point_to_point_movement(rcst_comm: Communication):
    ...
    for i, (start_x, start_y, target_x, target_y, max_error_mm) in enumerate(movement_tests):
        print(f"Movement Test {i+1}: ({start_x}, {start_y}) -> ({target_x}, {target_y})")
    ...
    print(f"Movement accuracy success rate: {successful_movements}/{total_movements} ({success_rate:.1%})")
...
def test_rotation_accuracy(rcst_comm: Communication):
    ...
    print(f"Rotation accuracy success rate: {successful_rotations}/{total_rotations} ({success_rate:.1%})")
    ...
    if len(velocities) >= 20:  # 十分なデータポイントがある場合
        ...
        if len(recent_positions) >= 2:
            ...
            if dt > 0:
                ...
                assert 0.1 <= estimated_speed <= 3.0, f"Speed out of reasonable range: {estimated_speed}"
...
if __name__ == "__main__":
    ...
    print(f"\n=== Overall Results ===")
    print(f"Total success rate: {total_success}/{total_tests} ({overall_success_rate:.1%})")
