...
def test_ball_possession_control(rcst_comm: Communication):
    ...
    print(
        f"Ball possession time: {possession_time:.1f}s / {test_duration}s ({possession_rate:.1%})"
    )

    # 最低60%の時間でボールを制御下に置くことを要求
    assert possession_rate >= 0.6, (
        f"Ball possession rate insufficient: {possession_rate:.1%}"
    )
    ...
    if len(ball_positions) >= 10 and len(robot_positions) >= 10:
        ...
        for i in range(total_measurements):
            ...
            distance = calc.distance(
                ball_pos[0], ball_pos[1], robot_pos[0], robot_pos[1]
            )
        ...
        print(
            f"Dribbling control rate: {close_control_count}/{total_measurements} ({control_rate:.1%})"
        )

        # 最終位置の評価
        final_ball_pos = ball_positions[-1]
        distance_to_target = calc.distance(
            final_ball_pos[0], final_ball_pos[1], end_x, end_y
        )
        print(f"Final distance to target: {distance_to_target:.2f}m")

        # 評価基準
        assert control_rate >= 0.7, (
            f"Dribbling control rate insufficient: {control_rate:.1%}"
        )
        assert distance_to_target <= 0.5, (
            f"Did not reach target location: {distance_to_target:.2f}m"
        )
...
def test_ball_passing_accuracy(rcst_comm: Communication):
    ...
    rcst_comm.send_yellow_robot(
        1, receiver_x, receiver_y, math.radians(180)
    )  # レシーバー
    ...
    while time.time() - pass_start_time < max_pass_time:
        ...
        # ボールが初期位置から移動したかチェック
        if not ball_moving:
            distance_from_start = calc.distance(
                ball.x, ball.y, ball_initial_pos[0], ball_initial_pos[1]
            )
    ...
    assert final_distance_to_receiver <= 0.6, (
        f"Pass accuracy insufficient: {final_distance_to_receiver:.2f}m"
    )
...
def test_ball_shooting_power(rcst_comm: Communication):
    ...
    # 評価基準
    assert distance_from_start >= 1.0, (
        "Ball did not move significantly (shot not executed)"
    )
