...
def test_multiple_robots_ball_distance(rcst_comm: Communication):
    ...
    rcst_comm.send_yellow_robot(0, ball_x + 0.3, ball_y, math.radians(0))  # 300mm - 違反
    rcst_comm.send_yellow_robot(1, ball_x - 0.3, ball_y, math.radians(180))  # 300mm - 違反
    rcst_comm.send_yellow_robot(2, ball_x, ball_y + 0.4, math.radians(270))  # 400mm - 違反
    rcst_comm.send_yellow_robot(3, ball_x, ball_y - 0.4, math.radians(90))  # 400mm - 違反
    rcst_comm.send_yellow_robot(4, ball_x + 0.6, ball_y, math.radians(0))  # 600mm - 合法
    ...
    # 違反が検出されなかったことを確認
    violation_detected = rcst_comm.observer.customized().get_result("robots_too_close_to_ball")
    if violation_detected:
        print("WARNING: Multiple robots were detected too close to ball during the test")
