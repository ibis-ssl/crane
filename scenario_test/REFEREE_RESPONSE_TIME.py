...
def test_stop_command_response(rcst_comm: Communication):
        ...
        for robot_id, robot in yellow_robots.items():
            if robot_id not in response_times and robot_id in positions_before:
                ...
                if time_diff >= 0.5:  # 0.5秒以上経過している場合のみ速度計算
                    distance_moved = calc.distance(robot.x, robot.y, prev_pos[0], prev_pos[1])
                    ...
                    if speed <= 0.1:
                        ...
                        print(f"Robot {robot_id} stopped in {response_time:.3f}s (speed: {speed:.3f} m/s)")
    ...
    if response_times:
        ...
        assert max_response_time_actual <= 2.0, f"Some robots too slow to respond: {max_response_time_actual:.3f}s"
...
def test_start_command_response(rcst_comm: Communication):
            ...
            if robot_id not in response_times and robot_id in positions_before:
                ...
                if time_diff >= 0.3:  # 0.3秒以上経過してから判定
                    ...
                    if speed >= 0.2:
                        ...
                        response_times[robot_id] = response_time
                        print(f"Robot {robot_id} started moving in {response_time:.3f}s (speed: {speed:.3f} m/s)")
    ...
    if response_times:
        ...
        # START応答時間の評価基準
        assert avg_response_time <= 1.0, f"Average start response time too slow: {avg_response_time:.3f}s"
        assert max_response_time_actual <= 2.0, f"Some robots too slow to start: {max_response_time_actual:.3f}s"
...
def test_free_kick_command_response(rcst_comm: Communication):
    ...
    rcst_comm.send_blue_robot(1, ball_x + 0.4, ball_y + 0.2, math.radians(180))  # 近すぎる
    ...
    while time.time() - free_kick_time < max_response_time:
        ...
        for robot_id, robot in blue_robots.items():
            if robot_id not in retreat_response_times and robot_id in blue_positions_before:
                ...
                if distance_to_ball >= 0.5:
                    ...
                    print(f"Blue robot {robot_id} retreated in {response_time:.3f}s (distance: {distance_to_ball:.3f}m)")
    ...
    while time.time() - kick_detection_start < 3.0:
        ...
        ball_movement = calc.distance(ball.x, ball.y, initial_ball_pos[0], initial_ball_pos[1])
    ...
    if retreat_response_times:
        avg_retreat_time = sum(retreat_response_times.values()) / len(retreat_response_times)
        ...
        print(f"Blue robots retreated: {len(retreat_response_times)}/{len(blue_positions_before)}")
        
        # 評価基準
        assert avg_retreat_time <= 3.0, f"Retreat response too slow: {avg_retreat_time:.3f}s"
...
def test_game_state_transition_response(rcst_comm: Communication):
    ...
    # 評価基準
    assert avg_transition_time <= 2.0, f"State transitions too slow: {avg_transition_time:.3f}s"
    assert max_transition_time <= 3.0, f"Some transitions too slow: {max_transition_time:.3f}s"
...
if __name__ == "__main__":
    ...
    print(f"State transitions - Average: {transition_avg:.3f}s, Max: {transition_max:.3f}s")
