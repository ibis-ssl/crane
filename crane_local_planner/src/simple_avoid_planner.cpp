...
#include "crane_local_planner/simple_avoid_planner.hpp"

namespace crane
{
SimpleAvoidPlanner::SimpleAvoidPlanner(rclcpp::Node & node) : logger(node.get_logger())
{
...
std::vector<Point> SimpleAvoidPlanner::getAvoidancePoints(
...
std::optional<Point> SimpleAvoidPlanner::getAvoidancePoint(
...
std::optional<Point> SimpleAvoidPlanner::getAvoidancePoint(
  ...
  WorldModelWrapper::SharedPtr world_model, int depth)
{
  //    Point target;
  //    target << command.target_x.front(), command.target_y.front();
  //    auto robot = world_model->getOurRobot(command.robot_id);
...
void SimpleAvoidPlanner::filterAvoidancePointsByPlace(
...
void SimpleAvoidPlanner::filterAvoidancePointsByPath(
  std::vector<Point> & points, std::shared_ptr<RobotInfo> robot,
  crane_msgs::msg::LocalPlannerConfig config, Point from, WorldModelWrapper::SharedPtr world_model)
{
  points.erase(
    std::remove_if(
      points.begin(), points.end(),
...
crane_msgs::msg::RobotCommands SimpleAvoidPlanner::calculateRobotCommand(
