...
| キック速度 | `KickerModel` と校正済み設定から指令値を決める。校正手順は [ボール・キッカーの校正](./ball_model_calibration_guide.md) を参照する。 | [`kicker_physics.yaml`](https://github.com/ibis-ssl/crane/blob/develop/crane_world_model_publisher/config/kicker_physics.yaml)、[`KickerModel`](https://github.com/ibis-ssl/crane/blob/develop/utility/crane_physics/include/crane_physics/kicker_model.hpp) |
