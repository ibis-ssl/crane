...
`crane_local_planner/src/rvo2_planner.cpp` の `adjustForPenaltyAreaAvoidance` および物理ブレーキング制約にて対応済み。

- STOP、STOP_PRE_*、FREE_KICK、BALL_PLACEMENT等のセットプレイ状態では `PENALTY_AREA_OFFSET_STOP = 0.3m` のマージンを適用（SSLルール要求0.2mを上回る）
- INPLAY / HALT では通常の `PENALTY_AREA_OFFSET = 0.1m` を適用
- ロボットがペナルティエリア＋マージン内に入った場合は、ゴールから離れる方向に自動的に押し出す脱出処理が発動する

### 非対応可能性

- `planning_factors` が `(empty)` の状態で長時間停滞する場合、脱出処理のレスポンスが遅れる可能性がある
- 攻撃後のセットプレイ移行時に2秒の猶予内に退避できない高速侵入ケース
