...
`SSL_WrapperPacket` を受信し、`Detection`（ロボット・ボール位置）と `Geometry`（フィールド寸法）を処理して、`crane_world_model_publisher` へ供給する。

### SSL Referee Protocol

`Referee` メッセージを受信し、ゲーム状態・チーム情報・ゲームイベントをROS 2トピックとして配信する。
...
crane_session_coordinator → Strategy Selection
```
