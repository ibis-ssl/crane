...
### SelectionHysteresis<IdType>（汎用選択安定化）

ID付きの選択候補（ロボットID、プレイID等）の切替をヒステリシスで安定化する汎用テンプレートクラス（`selection_hysteresis.hpp`）。

- **保持時間制御**: 最小保持期間内は切替を抑制
- **改善率ベース判定**: スコア改善率が閾値を超えた場合のみ切替
- **緊急切替**: 大幅なスコア差がある場合は即座に切替
- **タイムアウト強制切替**: 長時間経過後は小改善でも切替を許可

### RonarEventDetector（イベント検出）

- **目的**: 試合中の重要なイベント（ゴール、シュート、パスカット等）を検出
- **用途**: 試合のリアルタイム分析と外部システムへのイベント通知に使用
