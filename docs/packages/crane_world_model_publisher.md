...
- **BallPhysicsModel** (`crane_physics`パッケージで定義): 3D物理ボールモデル
...
- **BallCalibrationDataExtractor**: ボールモデル学習用データ抽出
  - 観測されたボール軌道から物理パラメータ推定用のデータを抽出・保存

- **SimpleBallPhysicsOptimizer**: 簡易物理パラメータ最適化
  - 収集データに基づき、転がり抵抗係数などのパラメータを最適化
