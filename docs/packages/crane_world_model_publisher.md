...
- **チャタリング抑制**: 可視性スコア(0.0-1.0)・ヒステリシス閾値
...
- **チャタリング抑制**: ヒステリシス閾値(0.6/0.4)による安定した検出状態管理
...
このパッケージはVision処理システムの健全性を監視する診断情報を提供します：

### 提供する診断項目

- **Vision処理状態** (`vision/processing`)
  - Vision GCからのデータ受信状態
  - データ受信タイムアウト（1秒）の監視

### 診断レベル

- **ERROR**: 1秒以上Visionデータ未受信
- **OK**: Vision処理が正常に実行中

詳細は[診断システムドキュメント](../diagnostics.md#crane_world_model_publisher)を参照してください。
