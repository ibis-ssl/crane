...
namespace crane
{
void EmplaceRobotSession::onRobotsChanged() { m_skill_map.clear(); }
...
auto EmplaceRobotSession::sendSpeakGoal(const std::string & text) -> void
{
  if (!speak_client_) {
    return;
  }
...
auto EmplaceRobotSession::findAvailableSoundFile() -> std::string
{
  // 複数の候補パスを試す（優先順位順）
...
EmplaceRobotSession::calculatePositionCommand(const std::vector<RobotIdentifier> & robots)
{
  // ロボット退場中の警告
  if (!robots.empty()) {
