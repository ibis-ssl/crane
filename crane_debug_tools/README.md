...
- スタンドアローン版：コマンド生成とパラメータ設定
- フル版：ロボット位置とボール追跡のリアルタイム可視化（開発中）
...
3. **インターフェースの使用開始：**
   - 左パネルからスキルを選択
   - ロボット ID とパラメータを設定
   - "コマンド生成" をクリックしてCLIコマンドを生成
   - 生成されたコマンドをコピーしてターミナルで実行

#### フル版（開発中）
リアルタイムデータ統合が必要な場合：
```bash
ros2 launch crane_debug_tools debug_tools.launch.py enable_web:=true web_port:=8080
```
