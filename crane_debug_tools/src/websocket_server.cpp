...
public:
    ...
    WebSocketDebugServer() : Node("websocket_debug_server"), port_(8090), websocket_port_(8091)
    {
        ...
        session_injection_pub_ = this->create_publisher<std_msgs::msg::String>(
            "/session_injection", 10);
...
private:
    ...
    void handleWebSocketMessage(std::shared_ptr<WebSocketConnection> connection, const std::string& message) {
        try {
    ...
    void handleSkillExecution(std::shared_ptr<WebSocketConnection> connection, const json& request) {
        try {
            ...
            send_goal_options.feedback_callback =
                [this, connection](const SkillExecutionClient::GoalHandle::SharedPtr &,
    ...
    void activateSimpleAI(std::shared_ptr<WebSocketConnection> connection) {
        ...
        json response = {
            {"type", "simple_ai_activated"},
            {"message", "SimpleAI session activated"}
        };
        connection->sendMessage(response.dump());
