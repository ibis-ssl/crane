...
public:
   ...
   * @return SessionCapacityリスト（存在しない場合はstd::nullopt）
   */
...
private:
  ...
  // セッション名 → SessionCapacityリストのマッピング
  std::unordered_map<std::string, std::vector<SessionCapacity>> robot_selection_priority_map_;
