...
async function fetchRobots() {
  try {
    const res = await fetch(`${API_BASE}/robots`, { cache: 'no-store' });
  ...
  try {
    ...
    await Promise.all(
      Array.from({ length: NUM_ROBOTS }, (_, id) =>
        fetch(`${API_BASE}/robots/${id}/start`, { method: 'POST' })
      )
    );
    await fetchRobots();
...
async function stopAll() {
  ...
  try {
    ...
    await Promise.all(
      Array.from({ length: NUM_ROBOTS }, (_, id) =>
        fetch(`${API_BASE}/robots/${id}/stop`, { method: 'POST' })
      )
    );
    await fetchRobots();
...
function showDisabled(message) {
  const banner = document.getElementById('rm-disabled-banner');
  const text = document.getElementById('rm-disabled-text');
  if (text) text.textContent = message;
  // .m3-offline-banner は既定 display:none。.visible を付けないと出ない
...
async function init() {
  ...
  try {
    ...
    // config すら取れない = サーバが居ない。ポーリングは始めない
    showDisabled('サーバに接続できません。web-debugger (8090) が起動しているか確認してください。');
    return;
  }
  ...
  if (!enabled) {
    // sim では 192.168.20.0/24 にパケットを 1 つも出さない。
    // ここで setInterval を始めないことが、その担保のクライアント側の半分になる。
    showDisabled(
      'sim モードでは無効です。実機で使うには ./scripts/docker-dev.sh --robot-manager '
      + '（または ROBOT_MANAGER_ENABLED=1）で起動してください。');
    return;
  }
