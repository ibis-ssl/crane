  ...
  : Node("crane_field_analyzer", options)
{
  ...
  auto play_situation_callback = [this](const crane_msgs::msg::PlaySituation::SharedPtr msg) -> void {
    this->play_situation_callback(msg);
  };
  // FIXME トピック名を合わせる
  pub_role_scores_ = this->create_publisher<crane_msgs::msg::RoleScores>("~/role_scores", 10);
  sub_world_model_ = this->create_subscription<crane_msgs::msg::WorldModel>("crane_world_observer/world_model ", 10,
                                                                            world_model_callback);
  sub_play_situation_ = this->create_subscription<crane_msgs::msg::PlaySituation>("crane_play_switcher/play_situation",
                                                                                  10, play_situation_callback);
}
...
void FieldAnalyzerComponent::play_situation_callback(const crane_msgs::msg::PlaySituation::SharedPtr msg)
{
  ...
  if (msg->referee_id == PlaySituation::HALT || msg->referee_id == PlaySituation::STOP)
  {
    RoleScore score;
