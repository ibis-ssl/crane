...
def create_kick_plot(data: dict, output_path: str = None) -> None:
    ...
    if output_path is None:
        ...
        output_path = str(
            data_path.with_suffix(".png").with_name(
                data_path.stem.replace("_data", "_plot")
            )
        )
...
def print_data_summary(data: dict) -> None:
    ...
    print(
        f"  キック位置: ({event_info['kick_position']['x']:.3f}, {event_info['kick_position']['y']:.3f})"
    )
...
使用例:
  ...
  ros2 run crane_world_model_publisher plot_kick_events.py data.json --summary-only
    ...
    parser.add_argument(
        "--output", "-o", help="出力画像ファイルのパス（指定しない場合は自動生成）"
    )
    parser.add_argument(
        "--summary-only",
        action="store_true",
        help="グラフ生成をスキップしてデータ概要のみ表示",
    )
    parser.add_argument(
        "--no-display",
        action="store_true",
