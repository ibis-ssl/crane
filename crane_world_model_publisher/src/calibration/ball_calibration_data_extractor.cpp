  ...
  -> std::vector<KickDataPoint>
{
  ...
  try {
    ...
    while (reader.has_next()) {
      ...
      // ロボットコマンドの処理
        ...
        try {
          ...
          RCLCPP_WARN_ONCE(rclcpp::get_logger("BallCalibrationDataExtractor"), "ロボットコマンドのデシリアライゼーションに失敗: %s", e.what());
          // 続行して他のメッセージを処理
        }
      }
    }
    ...
    RCLCPP_INFO(rclcpp::get_logger("BallCalibrationDataExtractor"),
                "ROSBAGから抽出完了: ボールデータ %zu 個, ロボットコマンド %zu 個", 
...
) -> std::vector<KickDataPoint>
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("BallCalibrationDataExtractor"),
              ...
              config_.extract_straight_kicks_only ? "true" : "false",
              config_.min_kick_speed, config_.min_trajectory_points);
  
  for (const auto & [kick_time, kick_pos] : kick_events) {
    RCLCPP_INFO(rclcpp::get_logger("BallCalibrationDataExtractor"),
                "キックイベントマッチング: 時刻=%.3fs, 位置=(%.3f,%.3f)", 
                kick_time.seconds(), kick_pos.x(), kick_pos.y());
    ...
    if (cmd_it != command_data.end()) {
      ...
      RCLCPP_INFO(rclcpp::get_logger("BallCalibrationDataExtractor"),
                  "最も近いコマンド: ロボット%u, 時刻=%.3fs, 時間差=%.3fs, キック力=%.3f",
                  cmd_msg.robot_id, cmd_time.seconds(), time_diff, cmd_msg.kick_power);
                  
      if (time_diff <= 0.5) { // 500ms以内
        RCLCPP_INFO(rclcpp::get_logger("BallCalibrationDataExtractor"),
                    "時間差OK: %.3fs以内", time_diff);
        ...
        if (!config_.target_robot_ids.empty() && 
          ...
          RCLCPP_WARN(rclcpp::get_logger("BallCalibrationDataExtractor"),
        ...
        RCLCPP_INFO(rclcpp::get_logger("BallCalibrationDataExtractor"),
                    ...
                    trajectory_quality_ok ? "OK" : "NG", 
                    kick_point.max_speed, config_.min_kick_speed, min_speed_ok ? "OK" : "NG",
                    kick_point.trajectory.size());
        
        if (trajectory_quality_ok && min_speed_ok) {
          kick_points.push_back(kick_point);
          RCLCPP_INFO(rclcpp::get_logger("BallCalibrationDataExtractor"),
                      "キックデータポイント追加: ロボット%u, 最高速度=%.3fm/s",
                      kick_point.kicker_id, kick_point.max_speed);
        } else {
          RCLCPP_WARN(rclcpp::get_logger("BallCalibrationDataExtractor"),
                      "キックデータポイント除外: 品質=%s, 速度=%s",
                      trajectory_quality_ok ? "OK" : "NG", min_speed_ok ? "OK" : "NG");
        }
      } else {
        RCLCPP_WARN(rclcpp::get_logger("BallCalibrationDataExtractor"),
  ...
  for (size_t i = 1; i < ball_data.size() - 1; ++i) {
    ...
    if (speed_increase > 0.5 || speed_ratio > 1.0 || curr_speed > config_.min_kick_speed * 0.5) {
      ...
      if (i % 100 == 0) {  // 100個に1回ログ出力
        RCLCPP_DEBUG(rclcpp::get_logger("BallCalibrationDataExtractor"),
                     "候補 #%zu: 前速度=%.3f, 現速度=%.3f, 次速度=%.3f, 増加=%.3f, 比率=%.3f",
                     i, prev_speed, curr_speed, next_speed, speed_increase, speed_ratio);
      }
    }
    ...
    if (speed_increase_ok && speed_ratio_ok && min_speed_ok) {
      ...
      RCLCPP_INFO(rclcpp::get_logger("BallCalibrationDataExtractor"),
                  ...
                  curr_time.seconds(), curr_ball.pos.x(), curr_ball.pos.y(), curr_speed, speed_increase, speed_ratio);
    }
  }
