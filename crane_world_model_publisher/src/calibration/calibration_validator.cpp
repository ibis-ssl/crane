...
auto CalibrationValidator::setConfig(const ValidatorConfig & config) -> void { config_ = config; }
  ...
  const SimpleKickerCalibrator::KickerModel & kicker_model) -> ValidationMetrics
{
  ...
  if (kick_data.size() < config_.min_validation_samples) {
    RCLCPP_WARN(
      rclcpp::get_logger("CalibrationValidator"), "検証データ不足: %zu個 (最小必要数: %zu)",
      kick_data.size(), config_.min_validation_samples);
    return metrics;
  }

  RCLCPP_INFO(
    rclcpp::get_logger("CalibrationValidator"), "キャリブレーション検証開始: %zu個の軌道を検証",
    kick_data.size());
  ...
  for (const auto & kick_point : kick_data) {
    ...
    double predicted_stop_time =
      physics_model.getStopTime(initial_ball.vel, initial_ball.state, initial_ball.vel_z);
    ...
    double predicted_max_distance = physics_model.getMaxDistance(
      initial_ball.pos, initial_ball.vel, initial_ball.state, initial_ball.pos_z,
      initial_ball.vel_z);
    ...
    double predicted_speed =
      kicker_model.linear_coefficient * kick_point.kick_power + kicker_model.offset;
  ...
  if (!position_errors.empty()) {
    double sum_squared = std::accumulate(
      position_errors.begin(), position_errors.end(), 0.0,
      [](double sum, double err) { return sum + err * err; });
    metrics.position_rmse = std::sqrt(sum_squared / position_errors.size());
    metrics.mean_absolute_error =
      std::accumulate(position_errors.begin(), position_errors.end(), 0.0) / position_errors.size();
    metrics.max_absolute_error = *std::max_element(position_errors.begin(), position_errors.end());
  }

  if (!stop_time_errors.empty()) {
    metrics.stop_time_error =
      std::accumulate(stop_time_errors.begin(), stop_time_errors.end(), 0.0) /
      stop_time_errors.size();
  }

  if (!max_distance_errors.empty()) {
    metrics.max_distance_error =
      std::accumulate(max_distance_errors.begin(), max_distance_errors.end(), 0.0) /
      max_distance_errors.size();
  }

  if (!speed_prediction_errors.empty()) {
    metrics.speed_prediction_error =
      std::accumulate(speed_prediction_errors.begin(), speed_prediction_errors.end(), 0.0) /
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("CalibrationValidator"),
    "検証完了: RMSE=%.3f m, 速度誤差=%.3f m/s, 精度=%.1f%% (%s)", metrics.position_rmse,
    metrics.speed_prediction_error, metrics.prediction_accuracy * 100.0,
...
auto CalibrationValidator::compareStopTimes(
  const std::vector<Ball> & actual_trajectory, double predicted_stop_time) -> double
{
  // 実際の停止時間を軌道から推定
  ...
  const SimpleKickerCalibrator::CalibrationResult & kicker_result) -> QualityReport
{
  ...
  // 総合品質評価
  report.overall_quality_good = metrics.validation_passed && physics_result.r_squared > 0.8 &&
  ...
  const SimpleKickerCalibrator::CalibrationResult & kicker_result) -> std::vector<std::string>
{
  ...
  if (metrics.position_rmse > 0.15) {
    recommendations.push_back(
      "位置予測精度が低下しています。より多くの高品質な軌道データを収集することを推奨します。");
  }

  // 物理モデルの決定係数評価
  if (physics_result.r_squared < 0.8) {
    recommendations.push_back(
      "物理モデルの当てはまりが悪いです。データの外れ値除去や追加パラメータの調整を検討してください"
      "。");
  }
  ...
  if (kicker_result.straight_kick_model.r_squared < 0.7) {
    recommendations.push_back(
      "キッカーパワー-"
      "速度関係の線形性が低いです。より広範囲のパワー設定でのデータ収集が必要です。");
  }

  // データ量の評価
  if (physics_result.data_points_used < 20) {
    recommendations.push_back(
      "最適化に使用されたデータ点数が少ないです。より多くのキックデータを収集してください。");
  }

  // 速度予測精度の評価
  if (metrics.speed_prediction_error > 0.5) {
    recommendations.push_back(
      "キック速度の予測精度が低下しています。キッカーの機械的なばらつきや測定精度を確認してください"
      "。");
  }

  if (recommendations.empty()) {
    recommendations.push_back(
      "キャリブレーション品質は良好です。現在のパラメータを本番環境に適用できます。");
  }
