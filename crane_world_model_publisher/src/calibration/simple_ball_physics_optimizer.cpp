...
auto SimpleBallPhysicsOptimizer::optimizeDecelerationParameter(
  const std::vector<KickDataPoint> & kick_data) -> OptimizationResult
{
  OptimizationResult result;

  if (kick_data.empty()) {
    RCLCPP_WARN(rclcpp::get_logger("SimpleBallPhysicsOptimizer"), "キックデータが空です");
    return result;
  }
  ...
  std::copy_if(
    kick_data.begin(), kick_data.end(), std::back_inserter(straight_kicks),
    [](const KickDataPoint & kd) { return !kd.is_chip_kick; });

  if (straight_kicks.empty()) {
    RCLCPP_WARN(
      rclcpp::get_logger("SimpleBallPhysicsOptimizer"), "ストレートキックデータが見つかりません");
    return result;
  }
  ...
  if (all_trajectory_points.size() < 10) {
    RCLCPP_WARN(
      rclcpp::get_logger("SimpleBallPhysicsOptimizer"), "軌道データ点数が不足: %zu点",
      all_trajectory_points.size());
    return result;
  }
  ...
  // 粗い検索
  for (double decel = config_.min_deceleration; decel <= config_.max_deceleration; decel += 0.05) {
    ...
    for (const auto & kick_point : straight_kicks) {
      ...
      auto predicted_trajectory =
        predictRollingTrajectory(initial_position, initial_ball.vel, decel, time_points);
  ...
  for (double decel = best_deceleration - 0.05; decel <= best_deceleration + 0.05;
       decel += fine_step) {
  ...
  for (const auto & kick_point : straight_kicks) {
    ...
    auto predicted_trajectory = predictRollingTrajectory(
      kick_point.trajectory[0].pos, kick_point.trajectory[0].vel, best_deceleration, time_points);
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("SimpleBallPhysicsOptimizer"),
    "最適化完了: 減速度=%.4f m/s², 誤差=%.4f m, R²=%.3f", result.optimized_deceleration,
    result.residual_error, result.r_squared);
  ...
  -> CalibrationResult
{
  ...
  if (straight_power_speed.size() >= 3) {
    ...
    RCLCPP_INFO(
      rclcpp::get_logger("SimpleKickerCalibrator"),
      "ストレートキック: slope=%.2f, offset=%.2f, R²=%.3f (%zu points)",
      result.straight_kick_model.linear_coefficient, result.straight_kick_model.offset,
      result.straight_kick_model.r_squared, straight_power_speed.size());
  }
  ...
  if (chip_power_speed.size() >= 3) {
    ...
    RCLCPP_INFO(
      ...
      result.chip_kick_model.linear_coefficient, result.chip_kick_model.offset,
      result.chip_kick_model.r_squared, chip_power_speed.size());
  }
...
auto SimpleKickerCalibrator::performLinearRegression(
  const std::vector<std::pair<double, double>> & power_speed_pairs) -> KickerModel
{
  KickerModel model;
...
auto SimpleKickerCalibrator::calculateRequiredPower(double target_speed, const KickerModel & model)
  -> double
{
