  ...
  node_(node)
{
  ...
  node_.declare_parameter("calibration.data_collection_cycles", static_cast<int64_t>(data_collection_cycles_));
  ...
  data_collection_cycles_ = static_cast<size_t>(node_.get_parameter("calibration.data_collection_cycles").as_int());
  ...
  const std::vector<RobotIdentifier> & robots, PlannerContext & context)
{
  ...
  switch (current_state_) {
    case CollectorState::SETUP_POSITIONS:
      {
        // キッカーを目標位置に配置
        auto kicker_command = std::make_shared<RobotCommandWrapper>(
          "calibration_kicker", kicker_robot_id_, world_model);
    ...
    case CollectorState::KICK_PREPARATION:
      {
        // キックスキルの初期化
        if (!kicker_skill_) {
          auto kicker_command = std::make_shared<RobotCommandWrapper>("kick", kicker_robot_id_, world_model);
          ...
          RCLCPP_INFO(node_.get_logger(), "キック準備: パワー=%.2f (サイクル %zu/%zu)", 
                     kick_power, current_cycle_ + 1, data_collection_cycles_);
        }
    ...
    case CollectorState::BALL_RETRIEVAL:
      {
        // 球拾いスキルの初期化
        if (!retriever_skill_) {
          auto retriever_command = std::make_shared<RobotCommandWrapper>("receive", retriever_robot_id_, world_model);
    ...
    case CollectorState::CYCLE_COMPLETE:
      {
        ...
        if (current_cycle_ >= data_collection_cycles_) {
          RCLCPP_INFO(node_.get_logger(), "全サイクル完了 (%zu/%zu)", current_cycle_, data_collection_cycles_);
        ...
        current_state_ = CollectorState::SETUP_POSITIONS;
        state_start_time_ = current_time;
        RCLCPP_INFO(node_.get_logger(), "次のサイクル開始 (%zu/%zu)", current_cycle_ + 1, data_collection_cycles_);
...
{
  switch (state) {
    case CollectorState::SETUP_POSITIONS: return "SETUP_POSITIONS";
    case CollectorState::KICK_PREPARATION: return "KICK_PREPARATION";  
    case CollectorState::EXECUTING_KICK: return "EXECUTING_KICK";
    case CollectorState::WAITING_BALL_STOP: return "WAITING_BALL_STOP";
    case CollectorState::BALL_RETRIEVAL: return "BALL_RETRIEVAL";
    case CollectorState::RETURN_PASS: return "RETURN_PASS";
    case CollectorState::CYCLE_COMPLETE: return "CYCLE_COMPLETE";
