...
CatchBallPlanner::calculateRobotCommand(const std::vector<RobotIdentifier> & robots)
{
  ...
  for (const auto & robot : robots) {
    auto command = std::make_shared<crane::RobotCommandWrapper>(robot.robot_id, world_model);
    ...
    if (not intersections.empty() && world_model->ball.vel.norm() > 0.3f) {
      ...
      cmd->setTargetPosition(result.closest_point);
      ...
      if (
        ...
        //          phase = "ボールを待ち受ける";
        cmd->setTargetPosition(default_point);
        command->lookAtBall();
      }
    }
