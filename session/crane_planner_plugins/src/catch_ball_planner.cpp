...
CatchBallPlanner::calculateRobotCommand(const std::vector<RobotIdentifier> & robots)
{
  ...
  for (const auto & robot : robots) {
    ...
    Segment ball_line(ball_pos, ball_pos + ball.vel.normalized() * 20.f);
    auto intersections =
      getIntersections(ball_line, Segment{receive_line.first, receive_line.second});

    if (not intersections.empty() && ball.vel.norm() > 0.3f) {
      // シュートブロック
      ...
      command->setTargetTheta(getAngle(-ball.vel));
      if (command->getRobot()->getDistance(result.closest_point) > 0.2) {
        command->setTerminalVelocity(2.0);
      }
    } else {
      if (ball.isStopped(0.3) && not world_model->point_checker.isFriendPenaltyArea(ball_pos)) {
        ...
        std::erase_if(passable_robot_list, [&](const RobotInfo::SharedPtr & r) {
          // 敵に塞がれていたら除外
          Segment ball_to_robot_line(ball_pos, r->pose.pos);
        ...
        Point intermediate_point = computeAroundBallApproachTargetDynamic(
          ball_pos, pass_target, command->getRobot()->pose.pos, INTERVAL, MAX_INTERVAL);
        double angle_ball_to_target = getAngle(pass_target - ball_pos);
        double dot = (ball_pos - command->getRobot()->pose.pos)
                       .normalized()
                       .dot((pass_target - ball_pos).normalized());
        ...
        if (
          ...
          command->setTargetPosition(ball_pos);
          command->kickStraight(0.1).disableCollisionAvoidance();
          command->enableCollisionAvoidance();
          command->disableBallAvoidance();
        }
        command->setTargetTheta(getAngle(pass_target - ball_pos));
        command->disableGoalAreaAvoidance();
      } else {
        //          phase = "";
        [[maybe_unused]] const double BLOCK_DIST = 0.15;
        // 範囲外のときは正面に構える
        if (not world_model->point_checker.isFieldInside(ball_pos)) {
          //            phase += "正面で";
          ball_pos << 0, 0;
        }
