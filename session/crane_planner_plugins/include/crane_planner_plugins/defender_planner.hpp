...
public:
    ...
    const std::vector<RobotIdentifier> & robots) override
  {
    ...
    std::vector<Point> defense_points =
      getDefensePoints(robots.size(), ball_line);
    ...
    const int robot_num, const Segment & ball_line) const
  {
    ...
    if(auto defense_patrameter = getDefenseLinePointParameter(ball_line)){
      ...
      auto add_parameter = [&](double parameter) -> bool {
        ...
        auto [threshold1, threshold2, threshold3] = getDefenseLinePointParameterThresholds(OFFSET_X, OFFSET_Y);
  ...
  auto getDefenseLinePoint(double parameter) const -> Point{
    ...
    const auto [threshold1, threshold2, threshold3] = getDefenseLinePointParameterThresholds(OFFSET_X, OFFSET_Y);
  ...
  auto getDefenseLinePointParameterThresholds(double offset_x, double offset_y) const -> std::tuple<double, double, double>{
  ...
  auto getDefenseAreaCorners(double offset_x, double offset_y) const -> std::tuple<Point, Point,Point,Point > {
    // デフェンスエリアを囲みし4つの点
    Point p1, p2, p3, p4;
  ...
  auto getDefenseLinePointParameter(Segment target_segment) const -> std::optional<double> {
    ...
    if(bg::intersection(Segment{p1, p2}, target_segment, intersections); not intersections.empty()){
      return std::abs(intersections[0].x() - p1.x());
    }else if (bg::intersection(Segment{p2, p3}, target_segment, intersections); not intersections.empty()) {
      return std::abs(intersections[0].y() - p2.y()) + threshold1;
    }else if (bg::intersection(Segment{p3, p4}, target_segment, intersections); not intersections.empty()) {
