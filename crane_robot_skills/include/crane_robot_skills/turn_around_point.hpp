 ...
 */
    ...
    public:
        explicit TurnAroundPoint(Point point, double angle, uint8_t id, std::shared_ptr<WorldModelWrapper> &world_model)
                : SkillBase<>("turn_around_point", id, world_model, DefaultStates::DEFAULT), target_point(point),
                  target_angle(angle) {
            addStateFunction(
                    ...
                    [this](
                        ...
                        if (std::abs(getAngleDiff(getAngle(robot->pose.pos - target_point), target_angle)) < 0.1) {
                            ...
                            Velocity velocity = ((target_point - robot->pose.pos).normalized() * (dr)) +
