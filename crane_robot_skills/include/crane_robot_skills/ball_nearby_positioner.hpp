...
public:
  ...
  Status update([[maybe_unused]] const ConsaiVisualizerWrapper::SharedPtr & visualizer) override
  {
    ...
    double normalized_offset =
      (getParameter<int>("total_robot_number") - getParameter<int>("robot_index") - 1) / 2.;
    double offset = normalized_offset * getParameter<double>("robot_interval");
    Point base_position = world_model()->ball.pos + [&](const std::string & policy) {
      if (policy == "goal") {
        ...
        throw std::runtime_error(
          "[BallNearByPositioner] 予期しないパラメータ「positioning policy」が入力されています: " +
          policy);
      }
    }(getParameter<std::string>("line_policy")) * distance_from_ball;
    ...
    Point target_position = [&](const std::string & policy) -> Point {
      ...
      if (policy == "arc") {
        ...
        throw std::runtime_error(
          "[BallNearByPositioner] 予期しないパラメータ「line policy」が入力されています: " +
          policy);
      }
