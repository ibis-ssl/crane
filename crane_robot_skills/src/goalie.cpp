...
void Goalie::emitBallFromPenaltyArea()
{
  ...
  auto passable_robot_list =
    world_model()->ours().getAvailableRobots(command->getMsg().robot_id) |
    ranges::views::filter([&](const auto & r) {
      return std::abs(r->pose.pos.x() - world_model()->getOurGoalCenter().x()) >=
               world_model()->getDefenseHeight() &&
             world_model()->getDistanceFromRobotToBall(r->getID()) >= 0.5;
    }) |
