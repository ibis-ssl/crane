...
: SkillBaseWithState<StealBallState>("StealBall", base, StealBallState::MOVE_TO_FRONT)
{
  ...
  addStateFunction(
    ...
    [this]([[maybe_unused]] const ConsaiVisualizerWrapper::SharedPtr & visualizer) -> Status {
      ...
      auto their_frontier = world_model()->getNearestRobotWithDistanceFromPoint(world_model()->ball.pos, world_model()->theirs.getAvailableRobots());
      if (method == "front") {
        ...
        if (robot()->getDistance(world_model()->ball.pos) < (0.085 - 0.030)) {
          command.setDribblerTargetPosition(world_model()->ball.pos + getVerticalVec(world_model()->ball.pos - robot()->pose.pos) * 0.3);
...
//          command.setTargetTheta(getAngle(world_model()->ball.pos - robot()->pose.pos) + M_PI / 2);
        ...
        if(world_model()->getTheirFrontier().has_value() && robot()->getDistance(world_model()->ball.pos) < world_model()->getTheirFrontier()->robot->getDistance(world_model()->ball.pos)){
