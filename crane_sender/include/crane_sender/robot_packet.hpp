...
auto convertTwoByteToFloat(uint8_t byte_high, uint8_t byte_low, float range)
  -> float
{
...
#define FLOAT_FROM_2BYTE(name, range)             \
  packet.name = convertTwoByteToFloat(            \
    data[static_cast<int>(Address::name##_HIGH)], \
    data[static_cast<int>(Address::name##_LOW)], range)
...
#define FLOAT_TO_2BYTE(name, range)                                         \
  std::pair<uint8_t, uint8_t> name##_two_byte =                             \
    convertFloatToTwoByte(name, range);                                     \
  serialized                                                                \
    .data[static_cast<int>(RobotCommandSerialized::Address::name##_HIGH)] = \
    name##_two_byte.first;                                                  \
  serialized                                                                \
    .data[static_cast<int>(RobotCommandSerialized::Address::name##_LOW)] =  \
    name##_two_byte.second

#define FLOAT_TO_1BYTE(name, range)                                          \
  uint8_t name##_one_byte = static_cast<uint8_t>(name / range * 255.0f);     \
  serialized.data[static_cast<int>(RobotCommandSerialized::Address::name)] = \
    name##_one_byte
