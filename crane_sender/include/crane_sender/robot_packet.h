...
struct RobotCommandSerializedV2
{
  ...
  void deserialize()
  {
    ...
    command.vision_global_x = convertTwoByteToFloat(
      data[Address::VISION_GLOBAL_X_HIGH], data[Address::VISION_GLOBAL_X_LOW], 32.767);
    command.vision_global_y = convertTwoByteToFloat(
      data[Address::VISION_GLOBAL_Y_HIGH], data[Address::VISION_GLOBAL_Y_LOW], 32.767);
    ...
    command.omega_limit = convertTwoByteToFloat(
      data[Address::OMEGA_LIMIT_HIGH], data[Address::OMEGA_LIMIT_LOW], 32.767);
    ...
    command.prioritize_accurate_acceleration =
      (flags >> FlagAddress::PRIORITIZE_ACCURATE_ACCELERATION) & 0x01;
    ...
    switch (command.control_mode) {
      case RobotCommandV2::ControlMode::LOCAL_CAMERA_MODE:
        ...
        break;
      case RobotCommandV2::ControlMode::POSITION_TARGET_MODE:
        command.position_target_mode_args =
          new PositionTargetModeArgs(&data[Address::CONTROL_MODE_ARGS]);
        break;
      case RobotCommandV2::ControlMode::SIMPLE_VELOCITY_TARGET_MODE:
        command.simple_velocity_target_mode_args =
          new SimpleVelocityTargetModeArgs(&data[Address::CONTROL_MODE_ARGS]);
        break;
      case RobotCommandV2::ControlMode::VELOCITY_TARGET_WITH_TRAJECTORY_MODE:
        command.velocity_target_with_trajectory_mode_args =
