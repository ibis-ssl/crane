...
def analyze_mcap(mcap_path: str):
    ...
    robot_checkpoint_values = defaultdict(lambda: defaultdict(list))  # valueフィールドの値
        ...
        for schema, channel, message in reader.iter_messages(topics=[target_topic]):
            ...
            try:
                ...
                if len(result['checkpoints']) > 1:
                    sorted_cps = sorted(result['checkpoints'], key=lambda x: x['relative_time_us'])
                    for i in range(1, len(sorted_cps)):
                        prev_cp = sorted_cps[i-1]
                        curr_cp = sorted_cps[i]
                        interval = curr_cp['relative_time_us'] - prev_cp['relative_time_us']
                ...
                for robot_id, robot_data in result['robot_checkpoints'].items():
                    for cp in robot_data['checkpoints']:
                        robot_checkpoint_times[cp['name']][robot_id].append(cp['relative_time_us'])
                        if cp['value']:
                            robot_checkpoint_values[cp['name']][robot_id].append(cp['value'])

                # 最初の数メッセージを表示
                if message_count <= 3:
                    print(f"=== Message #{message_count} ===")
                    print(f"  Timestamp: {result['timestamp_sec']}.{result['timestamp_nanosec']:09d}")
                    print(f"  Reference timestamp: {result['reference_timestamp_ns']} ns")
                    print(f"  VelocityCommands Checkpoints ({len(result['checkpoints'])}):")

                    sorted_cps = sorted(result['checkpoints'], key=lambda x: x['relative_time_us'])
                    for i, cp in enumerate(sorted_cps):
                        if i == 0:
                            print(f"    [{i+1}] {cp['name']}: {cp['relative_time_us']} us")
                        else:
                            prev = sorted_cps[i-1]
                            delta = cp['relative_time_us'] - prev['relative_time_us']
                            print(f"    [{i+1}] {cp['name']}: {cp['relative_time_us']} us (+{delta} us)")
                    ...
                    if result['robot_checkpoints']:
                        ...
                        for robot_id, robot_data in sorted(result['robot_checkpoints'].items()):
                            print(f"    Robot {robot_id}:")
                            for cp in robot_data['checkpoints']:
                                print(f"      - {cp['name']}: {cp['relative_time_us']} us", end='')
                                if cp['value']:
    ...
    for name in sorted(checkpoint_times.keys()):
        ...
        print(f"  平均: {statistics.mean(times):.2f} us ({statistics.mean(times)/1000:.3f} ms)")
        print(f"  中央値: {statistics.median(times):.2f} us ({statistics.median(times)/1000:.3f} ms)")
    ...
    for interval_key in sorted(checkpoint_intervals.keys()):
        ...
        print(f"  平均: {statistics.mean(intervals):.2f} us ({statistics.mean(intervals)/1000:.3f} ms)")
        print(f"  中央値: {statistics.median(intervals):.2f} us ({statistics.median(intervals)/1000:.3f} ms)")
    ...
    for data in all_delay_data:
        if data['checkpoints']:
            sorted_cps = sorted(data['checkpoints'], key=lambda x: x['relative_time_us'])
            # 最初から最後までの遅延
            total_delay = sorted_cps[-1]['relative_time_us'] - sorted_cps[0]['relative_time_us']
            total_delays.append(total_delay)

    if total_delays:
        print(f"\n総処理時間 (最初のチェックポイント → 最後のチェックポイント):")
        print(f"  平均: {statistics.mean(total_delays):.2f} us ({statistics.mean(total_delays)/1000:.3f} ms)")
        print(f"  中央値: {statistics.median(total_delays):.2f} us ({statistics.median(total_delays)/1000:.3f} ms)")
    ...
    if robot_checkpoint_times:
        ...
        for cp_name in sorted(robot_checkpoint_times.keys()):
            ...
            for robot_id in sorted(robot_checkpoint_times[cp_name].keys()):
                ...
                if cp_name in robot_checkpoint_values and robot_id in robot_checkpoint_values[cp_name]:
                    values = robot_checkpoint_values[cp_name][robot_id]
                    if values:
                        print(f"    value フィールドのサンプル (最初の5個):")
