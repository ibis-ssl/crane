...
inline auto getCircle(const Point & p1, const Point & p2, const Point & p3) -> std::optional<Circle>
{
  ...
  circle.center.x() = (p1_sq * (p2.y() - p3.y()) + p2_sq * (p3.y() - p1.y()) + p3_sq * (p1.y() - p2.y())) / D; // Corrected syntax
  circle.center.y() = (p1_sq * (p3.x() - p2.x()) + p2_sq * (p1.x() - p3.x()) + p3_sq * (p2.x() - p1.x())) / D; // Corrected syntax
