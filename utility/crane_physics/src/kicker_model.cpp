...
KickerModel::KickerModel(const Config & config) : config_(config), ball_physics_model_(nullptr)
{
  ...
  const bool straight_partial =
    (config_.straight_kick_powers.empty() ^ config_.straight_kick_speeds.empty());
  const bool chip_partial =
    (config_.chip_kick_powers.empty() ^ config_.chip_kick_distances.empty());
  const bool straight_mismatch =
    (!straight_empty && !straight_partial &&
     config_.straight_kick_powers.size() != config_.straight_kick_speeds.size());
  const bool chip_mismatch =
    (!chip_empty && !chip_partial &&
