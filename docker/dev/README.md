...
- ER-Force構成には実機CM4に相当する `cm4-sim` が含まれ、位置制御ループを閉じる。ホスト側のCraneは `planner:=visibility_graph ibis_target_port:=12345 feedback_sim_mode:=false` で起動する。`feedback_sim_mode:=false` を省くとCraneと `cm4-sim` が同じunicastポートを奪い合い、位置制御が動かない。詳細は[ネットワーク](../../docs/network.md)を参照する。
