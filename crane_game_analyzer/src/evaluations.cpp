  ...
  -> double
{
  ...
  auto cos_view = world_model->theirs().robots |
  ...
  auto max_cos = ranges::empty(cos_view) ? 0.0 : ranges::max(cos_view);
  // 角度が大きい(cosが小さい)ほど安全
  return 1 - max_cos;
}
...
{
  // 一番近い敵ロボットからの距離を求める
  auto sq_dist_view = world_model->theirs().robots |
                      ranges::views::filter([](const auto & enemy) { return enemy->available(); }) |
                      ranges::views::transform(
                        [&](const auto & enemy) { return (enemy->pose.pos - p).squaredNorm(); });
  auto min_sq_dist = ranges::empty(sq_dist_view) ? 100.0 : ranges::min(sq_dist_view);
