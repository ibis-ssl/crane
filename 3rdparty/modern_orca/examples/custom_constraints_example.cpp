...
public:
  SSL_BallAvoidanceConstraint(
    const Vector2D & ball_position, Scalar avoidance_distance = 0.5, int priority = 80)
  : ball_position_(ball_position), avoidance_distance_(avoidance_distance), priority_(priority)
  {
  }
...
public:
  SSL_PenaltyAreaConstraint(
    const Vector2D & penalty_center, const Vector2D & penalty_size, Scalar margin = 0.2,
    int priority = 90)
  : penalty_center_(penalty_center),
    penalty_size_(penalty_size),
    margin_(margin),
    priority_(priority)
  {
  }
...
{
public:
  DynamicFormationConstraint(
    const Vector2D & formation_center, Scalar formation_radius, Scalar time_period = 5.0,
    int priority = 30)
  : formation_center_(formation_center),
    formation_radius_(formation_radius),
    ...
    -> std::vector<HalfPlaneD> override
  {
    ...
    const auto target_offset = Vector2D{
      formation_radius_ * std::cos(rotation_angle), formation_radius_ * std::sin(rotation_angle)};
...
int main()
{
  ...
  for (int i = 0; i < 6; ++i) {
    ...
    const auto start_pos = Vector2D{3.0 * std::cos(angle), 3.0 * std::sin(angle)};
  ...
  for (int step = 0; step < 200; ++step) {
    ...
    const auto new_ball_pos =
      Vector2D{1.0 + 1.5 * std::sin(t * 0.5), 1.0 + 1.0 * std::cos(t * 0.3)};
    ...
    if (step % 30 == 0) {
      std::cout << "t=" << std::setw(4) << t << "s: Ball(" << new_ball_pos.x() << ","
                << new_ball_pos.y() << ") ";
