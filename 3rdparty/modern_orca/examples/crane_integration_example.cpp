...
class ROS2CompatibleAgent : public CircularAgent {
public:
    ROS2CompatibleAgent(AgentId id, const Vector2D& position, const Vector2D& preferred_velocity,
                       Scalar max_speed, Scalar radius, int robot_id = -1)
        : CircularAgent(id, position, preferred_velocity, max_speed, radius), 
          robot_id_(robot_id) {}
    
    auto getRobotId() const noexcept -> int { return robot_id_; }
    void setRobotId(int robot_id) { robot_id_ = robot_id; }
    
    auto clone() const -> std::unique_ptr<AgentBase<CircularCollisionModel>> override {
...
class CraneSSLFieldConstraint : public ConstraintBase<ROS2CompatibleAgent> {
public:
    CraneSSLFieldConstraint(Scalar field_length = 12.0, Scalar field_width = 9.0, 
                           Scalar boundary_margin = 0.2, int priority = 95)
        : field_length_(field_length), field_width_(field_width), 
          boundary_margin_(boundary_margin), priority_(priority) {}
    
    auto generateHalfPlanes(const ROS2CompatibleAgent& agent, TimeStep dt) const 
        ...
        if (pos.x() < -half_length + margin) {
            constraints.emplace_back(Vector2D{1.0, 0.0}, 
                                   Vector2D{-half_length + margin, pos.y()});
        }
        if (pos.x() > half_length - margin) {
            constraints.emplace_back(Vector2D{-1.0, 0.0}, 
                                   Vector2D{half_length - margin, pos.y()});
        ...
        if (pos.y() < -half_width + margin) {
            constraints.emplace_back(Vector2D{0.0, 1.0}, 
                                   Vector2D{pos.x(), -half_width + margin});
        }
        if (pos.y() > half_width - margin) {
            constraints.emplace_back(Vector2D{0.0, -1.0}, 
                                   Vector2D{pos.x(), half_width - margin});
    ...
    auto clone() const -> std::unique_ptr<ConstraintBase<ROS2CompatibleAgent>> override {
        return std::make_unique<CraneSSLFieldConstraint>(*this);
    }

private:
    Scalar field_length_;
    Scalar field_width_;
    Scalar boundary_margin_;
    int priority_;
};
...
public:
    CraneModernORCAPlanner() 
        : simulator_(std::make_unique<OptimalLinearProgram2DSolver>(4.0)) {
        simulator_.setParallelExecution(true);
    }
    
    auto addRobot(int robot_id, const Vector2D& position, Scalar max_speed = 3.0, 
                 Scalar radius = 0.09) -> AgentId {
        auto agent_id = simulator_.addAgent(position, Vector2D{0, 0}, max_speed, radius, robot_id);
    ...
    void setupMultiRobotORCA() {
        simulator_.addORCAConstraints();
    }
    
    void setTargetVelocity(AgentId agent_id, const Vector2D& target_velocity) {
        auto& agent = simulator_.getAgent(agent_id);
        agent.setPreferredVelocity(target_velocity);
    }
    
    void addBallAvoidance(const Vector2D& ball_position, Scalar avoidance_distance = 0.5) {
    ...
    void addPenaltyAreaAvoidance(const Vector2D& goal_center, bool is_our_goal = false) {
        const Vector2D penalty_size{1.8, 3.6};  // SSL penalty area size
        const Scalar margin = is_our_goal ? 0.1 : 0.2;  // Closer to our own goal
    ...
    auto step(TimeStep dt) -> std::vector<Vector2D> {
        simulator_.step(dt);
    ...
    auto getPositions() const -> std::vector<Vector2D> {
    ...
    auto getStatistics() const -> auto {
        return simulator_.getStatistics();
    }
...
private:
    Simulator<ROS2CompatibleAgent> simulator_;
    
    class SSL_BallAvoidanceConstraint : public ConstraintBase<ROS2CompatibleAgent> {
    public:
        SSL_BallAvoidanceConstraint(const Vector2D& ball_position, Scalar avoidance_distance)
    ...
    class SSL_PenaltyAreaConstraint : public ConstraintBase<ROS2CompatibleAgent> {
    public:
...
int main() {
    ...
    std::vector<AgentId> robot_agents;
    for (int i = 0; i < 6; ++i) {
    ...
    planner.addPenaltyAreaAvoidance(Vector2D{-6.0, 0.0}, true);   // Our goal
    planner.addPenaltyAreaAvoidance(Vector2D{6.0, 0.0}, false);  // Their goal
    
    planner.addBallAvoidance(Vector2D{0.0, 0.0}, 0.5);
    
    for (std::size_t i = 0; i < robot_agents.size(); ++i) {
        const auto target_velocity = Vector2D{
            1.5 * std::cos(2.0 * PI * i / 6.0),
            1.0 * std::sin(2.0 * PI * i / 6.0)
        };
    ...
    for (int step = 0; step < 300; ++step) {
        ...
        if (step % 60 == 0) {
            ...
            for (std::size_t i = 0; i < std::min(size_t(3), positions.size()); ++i) {
                std::cout << "R" << i << "(" << std::setw(5) << positions[i].x() 
                         << "," << std::setw(5) << positions[i].y() << ") ";
            }
