...
public:
    ...
    explicit Simulator(std::unique_ptr<SolverBase<solver_type>> solver = 
                      std::make_unique<LinearProgram2DSolver>())
    ...
    void removeAgent(AgentId id) {
        ...
        for (auto& [_, constraints] : agent_constraints_) {
            auto orca_constraints = constraints->template getConstraintsOfType<ORCAConstraint<AgentType>>();
    ...
    auto addGlobalConstraint(Args&&... args) -> std::vector<ConstraintType*> {
        ...
        for (auto& [id, _] : agents_) {
            added_constraints.push_back(
                addConstraint<ConstraintType>(id, std::forward<Args>(args)...));
        }
    ...
    void setSolver(std::unique_ptr<SolverBase<solver_type>> solver) {
...
private:
    ...
    void stepParallel(TimeStep dt) {
        ...
        std::transform(std::execution::par_unseq, 
                      agent_ids.begin(), agent_ids.end(),
                      velocity_updates.begin(),
