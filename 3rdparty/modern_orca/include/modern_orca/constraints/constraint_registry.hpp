...
public:
    ...
    static void registerConstraint(const std::string& name) {
        static_assert(std::is_base_of_v<ConstraintBase<AgentType>, ConstraintType>,
...
public:
    ...
    auto generateAllHalfPlanes(const AgentType& agent, TimeStep dt) const 
        -> std::vector<HalfPlaneD> {
        ...
        for (const auto& constraint : constraints_) {
            if (constraint->isActive()) {
                ...
                all_constraints.insert(all_constraints.end(), 
                                     half_planes.begin(), half_planes.end());
            }
        }
    ...
    auto getConstraints() const -> const std::vector<ConstraintPtr>& {
        return constraints_;
    }
    
    auto getConstraintsByName(const std::string& name) const -> std::vector<ConstraintBase<AgentType>*> {
