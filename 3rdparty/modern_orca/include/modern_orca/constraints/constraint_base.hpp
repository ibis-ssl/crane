...
public:
    BoundaryConstraint(const Vector2D& min_bounds, const Vector2D& max_bounds, 
                      Scalar margin = 0.1, int priority = 100)
    ...
    auto generateHalfPlanes(const AgentType& agent, TimeStep /*dt*/) const 
        ...
        if (pos.x() - radius < min_bounds_.x() + margin_) {
            constraints.emplace_back(Vector2D{1.0, 0.0}, 
                                   Vector2D{min_bounds_.x() + margin_ + radius, pos.y()});
        }
        
        if (pos.x() + radius > max_bounds_.x() - margin_) {
            constraints.emplace_back(Vector2D{-1.0, 0.0}, 
                                   Vector2D{max_bounds_.x() - margin_ - radius, pos.y()});
        }
        
        if (pos.y() - radius < min_bounds_.y() + margin_) {
            constraints.emplace_back(Vector2D{0.0, 1.0}, 
                                   Vector2D{pos.x(), min_bounds_.y() + margin_ + radius});
        }
        
        if (pos.y() + radius > max_bounds_.y() - margin_) {
            constraints.emplace_back(Vector2D{0.0, -1.0}, 
                                   Vector2D{pos.x(), max_bounds_.y() - margin_ - radius});
        }
...
public:
    ...
    auto generateHalfPlanes(const AgentType& agent, TimeStep /*dt*/) const 
        ...
        if (current_speed > max_speed_ + EPSILON) {
            ...
            constraints.emplace_back(-vel_normalized, 
                                   agent.position() + max_speed_ * vel_normalized);
        }
...
public:
    CircularObstacleConstraint(const Vector2D& center, Scalar radius, 
                              Scalar margin = 0.1, int priority = 75)
