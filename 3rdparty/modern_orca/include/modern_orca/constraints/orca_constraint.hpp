...
public:
    ORCAConstraint(const std::vector<AgentType*>& other_agents, 
                   Scalar time_horizon = 2.0, int priority = 10)
    ...
    auto generateHalfPlanes(const AgentType& agent, TimeStep /*dt*/) const 
        ...
        for (const auto* other_agent : other_agents_) {
            ...
            if (distance_sq > combined_radius_sq) {
                ...
                const auto time = (combined_radius * combined_radius - distance_sq) / 
                                 (2.0 * combined_radius);
    ...
    void addOtherAgent(AgentType* agent) {
        other_agents_.push_back(agent);
    }
    
    void removeOtherAgent(AgentType* agent) {
        other_agents_.erase(
            std::remove(other_agents_.begin(), other_agents_.end(), agent),
            other_agents_.end());
    }
...
public:
    VelocityObstacleConstraint(const AgentType* other_agent, 
                              Scalar time_horizon = 1.5, int priority = 20)
...
public:
    ReciprocalVelocityObstacleConstraint(const AgentType* other_agent, 
                                        Scalar time_horizon = 2.0, 
                                        Scalar responsibility_factor = 0.5,
                                        int priority = 15)
        : other_agent_(other_agent), time_horizon_(time_horizon),
          responsibility_factor_(responsibility_factor), priority_(priority) {}
