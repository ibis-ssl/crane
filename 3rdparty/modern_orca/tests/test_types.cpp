...
TEST_CASE("Vector2D basic operations", "[Vector2D]") {
    ...
    SECTION("Angle and rotation") {
        ...
        Vector2D v2(0.0, 1.0);
        REQUIRE(v2.angle() == Approx(PI / 2.0));
        
        auto rotated = v1.rotate(PI / 2.0);
...
TEST_CASE("HalfPlane operations", "[HalfPlane]") {
    SECTION("Construction and distance") {
        ...
        Vector2D test_point(3.0, 0.0);
        REQUIRE(plane.signedDistance(test_point) == Approx(1.0));
        
        Vector2D test_point2(1.0, 0.0);
        REQUIRE(plane.signedDistance(test_point2) == Approx(-1.0));
    }
    
    SECTION("Containment testing") {
        HalfPlaneD plane(Vector2D(0.0, 1.0), Vector2D(0.0, 1.0));
        
        REQUIRE(plane.contains(Vector2D(0.0, 2.0)));
        REQUIRE(plane.contains(Vector2D(0.0, 1.0)));
        REQUIRE_FALSE(plane.contains(Vector2D(0.0, 0.5)));
    }
    
    SECTION("Projection") {
        HalfPlaneD plane(Vector2D(1.0, 0.0), Vector2D(1.0, 0.0));
        Vector2D test_point(0.5, 2.0);
...
TEST_CASE("Line operations", "[Line]") {
    ...
    SECTION("Closest point and distance") {
        LineD line = LineD::fromTwoPoints(Vector2D(0.0, 0.0), Vector2D(2.0, 0.0));
        Vector2D test_point(1.0, 1.0);
