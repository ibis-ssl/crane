...
{
  crane::Vector2d normal(1.0, 0.0);
  crane::Vector2d point(2.0, 0.0);
  ...
  crane::Vector2d test_point(3.0, 0.0);
  EXPECT_NEAR(plane.signedDistance(test_point), 1.0, 1e-10);

  crane::Vector2d test_point2(1.0, 0.0);
  EXPECT_NEAR(plane.signedDistance(test_point2), -1.0, 1e-10);
}

TEST(HalfPlaneTest, ContainmentTest)
{
  crane::modern_orca::HalfPlane plane(crane::Vector2d(0.0, 1.0), crane::Vector2d(0.0, 1.0));

  EXPECT_TRUE(plane.contains(crane::Vector2d(0.0, 2.0)));
  EXPECT_TRUE(plane.contains(crane::Vector2d(0.0, 1.0)));
  EXPECT_FALSE(plane.contains(crane::Vector2d(0.0, 0.5)));
}

TEST(HalfPlaneTest, Projection)
{
  crane::modern_orca::HalfPlane plane(crane::Vector2d(1.0, 0.0), crane::Vector2d(1.0, 0.0));
  crane::Vector2d test_point(0.5, 2.0);
...
{
  crane::Vector2d point(0.0, 0.0);
  crane::Vector2d direction(1.0, 1.0);
...
TEST(LineTest, ClosestPointAndDistance)
{
  crane::modern_orca::Line line =
    crane::modern_orca::Line::fromTwoPoints(crane::Vector2d(0.0, 0.0), crane::Vector2d(2.0, 0.0));
  crane::Vector2d test_point(1.0, 1.0);
...
{
  crane::Vector2d a(0.0, 0.0);
  crane::Vector2d b(3.0, 4.0);
